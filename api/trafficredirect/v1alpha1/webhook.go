...
func (r *Rule) validateMac(m string) error {
	...
	if !matched {
		return fmt.Errorf("mac %s is invalid, only support [0-9a-f:]")
	}
	return nil
}
...
func (r *Rule) Default() {
	...
	r.Spec.Match.DstMac = strings.ReplaceAll(r.Spec.Match.DstMac, "-", ":")
	r.Spec.Match.SrcMac = strings.ReplaceAll(r.Spec.Match.SrcMac, "-", ":")
